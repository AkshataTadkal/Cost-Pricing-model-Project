...
# ══════════════════════════════════════════════════════════════════════
# CUSTOMER ONBOARDING — data access + validation + AI assistant helpers
# Reuses session, _wf_df, _wf_exec, _sql_escape, _uuid, get_current_user,
# ai_complete, ai_reason_score. NO Cost Model logic is implemented here.
# ══════════════════════════════════════════════════════════════════════
_OB = "PRICING_ENGINE_DB.CORE_INTERNAL"

# Session-state additions (only new keys; reuses your _init_state pattern)
_init_state("selected_onboarding_mode", None)     # 'INTEGRATED' / 'STANDALONE'
_init_state("workspace_id", None)
_init_state("onboarding_customer_info", {})
_init_state("onboarding_selected_customer", None)
_init_state("onboarding_selected_product", None)
_init_state("integration_ready", False)
_init_state("onboarding_completed", False)
_init_state("pending_main_nav_tab", None)
_init_state("standalone_pricing_inputs", None)
_init_state("onboarding_batch_id", None)
_init_state("onboarding_source_df", None)
_init_state("onboarding_source_name", None)
_init_state("onboarding_source_type", None)
_init_state("onboarding_source_hash", None)
_init_state("onboarding_sheet_name", None)

_init_state("onboarding_validation_results", [])
_init_state("onboarding_validation_summary", None)
_init_state("onboarding_column_mapping", {})
_init_state("onboarding_mapping_confirmed", False)

_init_state("onboarding_staging_complete", False)
_init_state("onboarding_promotion_complete", False)
_init_state("onboarding_ai_quality_result", None)


def _ob_id(prefix):
    return f"{prefix}-{_uuid.uuid4().hex[:10].upper()}"

def ob_log(workspace_id, event_type, detail, actor):
    """Best-effort onboarding audit — never blocks the UI."""
    _wf_exec(f"""
        INSERT INTO {_OB}.ONBOARDING_LOG
        (LOG_ID, WORKSPACE_ID, EVENT_TYPE, EVENT_DETAIL, ACTOR, CREATED_AT)
        VALUES ('{_ob_id("LOG")}','{_sql_escape(workspace_id or "")}',
                '{_sql_escape(event_type)}','{_sql_escape(detail)}',
                '{_sql_escape(actor)}',CURRENT_TIMESTAMP())
    """)

def ob_validate_object(fqtn):
    """REAL reachability check — returns 'connected' or 'missing'. Never faked."""
    try:
...
def ob_cortex_available():
    """REAL Cortex probe (reuses the model chosen in AI Diagnostics)."""
    try:
        m = st.session_state.get("ai_model_override") or "llama3.1-70b"
        r = session.sql(f"SELECT AI_COMPLETE('{m}', 'ping') AS R").collect()
        return bool(r and r[0]["R"])
    except Exception:
        return False

def ob_integration_status():
    """Reads INTEGRATION_CONFIG for the Cost Model. Returns dict or None
    (None => not configured). Never fabricates a successful integration."""
    df = _wf_df(f"SELECT * FROM {_OB}.INTEGRATION_CONFIG "
                f"WHERE UPPER(INTEGRATION_NAME)='COST_MODEL' LIMIT 1")
    if df.empty:
        return None
    row = df.iloc[0].to_dict()
    return row if row.get("IS_CONFIGURED") else {**row, "IS_CONFIGURED": False}

def ob_init_workspace(info, mode, user):
    """Create/refresh a workspace row. Returns (workspace_id, error)."""
    wid = st.session_state.get("workspace_id") or _ob_id("WS")
    _wf_exec(f"DELETE FROM {_OB}.CUSTOMER_WORKSPACES WHERE WORKSPACE_ID='{_sql_escape(wid)}'")
    ok, err = _wf_exec(f"""
        INSERT INTO {_OB}.CUSTOMER_WORKSPACES
        (WORKSPACE_ID, WORKSPACE_NAME, CUSTOMER_ID, CUSTOMER_NAME, INDUSTRY, REGION,
         BUSINESS_UNIT, COUNTRY, SALES_REP, CURRENCY, LANGUAGE, CUSTOMER_TYPE,
         ONBOARDING_MODE, WORKSPACE_STATUS, READINESS_SCORE, CREATED_BY,
         CREATED_AT, UPDATED_AT)
        VALUES ('{wid}','{_sql_escape(info.get("workspace_name",""))}',
                '{_sql_escape(info.get("customer_id",""))}','{_sql_escape(info.get("customer_name",""))}',
                '{_sql_escape(info.get("industry",""))}','{_sql_escape(info.get("region",""))}',
                '{_sql_escape(info.get("business_unit",""))}','{_sql_escape(info.get("country",""))}',
                '{_sql_escape(info.get("sales_rep",""))}','{_sql_escape(info.get("currency",""))}',
                '{_sql_escape(info.get("language",""))}','{_sql_escape(info.get("customer_type",""))}',
                '{_sql_escape(mode or "")}','INITIALIZED',0,'{_sql_escape(user)}',
                CURRENT_TIMESTAMP(),CURRENT_TIMESTAMP())
    """)
    if ok:
        ob_log(wid, "WORKSPACE_INIT", f"Initialized in {mode} mode", user)
    return (wid if ok else None), err

def ob_save_workspace(wid, status, readiness, product, user):
    """Persist final status + selected product + readiness. Returns (ok, err)."""
    ok, err = _wf_exec(f"""
        UPDATE {_OB}.CUSTOMER_WORKSPACES
        SET WORKSPACE_STATUS='{_sql_escape(status)}',
            READINESS_SCORE={float(readiness)}, UPDATED_AT=CURRENT_TIMESTAMP()
        WHERE WORKSPACE_ID='{_sql_escape(wid)}'
    """)
    if ok and product:
        _wf_exec(f"DELETE FROM {_OB}.WORKSPACE_PRODUCTS WHERE WORKSPACE_ID='{_sql_escape(wid)}'")
        _wf_exec(f"""
            INSERT INTO {_OB}.WORKSPACE_PRODUCTS
            (WORKSPACE_PRODUCT_ID, WORKSPACE_ID, SKU, PRODUCT_FAMILY, SELECTED_BY,
             SELECTED_AT, METADATA_JSON)
            VALUES ('{_ob_id("WSP")}','{_sql_escape(wid)}','{_sql_escape(product.get("sku",""))}',
                    '{_sql_escape(product.get("family",""))}','{_sql_escape(user)}',
                    CURRENT_TIMESTAMP(),'{_sql_escape(json.dumps(product))}')
        """)
        ob_log(wid, "WORKSPACE_SAVE", f"status={status}, readiness={readiness}", user)
    return ok, err

def ob_record_mode(wid, mode, user):
    _wf_exec(f"""
        INSERT INTO {_OB}.MODE_SELECTION
        (SELECTION_ID, WORKSPACE_ID, USER_NAME, SELECTED_MODE, CREATED_AT)
        VALUES ('{_ob_id("MODE")}','{_sql_escape(wid or "")}','{_sql_escape(user)}',
                '{_sql_escape(mode)}',CURRENT_TIMESTAMP())
    """)

def ob_ai_assistant(info, mode, product, checklist):
    """Advisory AI assistant. Reuses ai_reason_score. NEVER initializes,
    saves, or mutates onboarding data — read-only recommendation only."""
    facts = (
        f"Customer: {info.get('customer_name') or 'N/A'} ({info.get('customer_id') or 'N/A'})\n"
        f"Industry: {info.get('industry') or 'N/A'}, Region: {info.get('region') or 'N/A'}, "
        f"Country: {info.get('country') or 'N/A'}, Type: {info.get('customer_type') or 'N/A'}\n"
        f"Chosen mode: {mode or 'not selected'}\n"
        f"Selected product: {product.get('sku') if product else 'none'}\n"
        f"Checklist state: {json.dumps(checklist)}"
    )
    return ai_reason_score(
        module="customer_onboarding", decision_type="readiness_assessment",
        context_facts=facts,
        extra_instruction=(
            "You are an enterprise onboarding advisor. 'score' = onboarding readiness "
            "(0-100, higher = readier). In 'reasoning' give a short customer summary and a "
            "recommended onboarding mode. Put missing information and potential risks in 'risks', "
            "and concrete suggested next steps in 'recommended_actions'. Advisory only — you do "
            "not initialize or change anything."),
        cache_key=_ai_cache_key("ob_ai", info.get("customer_id"), mode,
...
# ══════════════════════════════════════════════════════════════════════
# CUSTOMER ONBOARDING DATA PIPELINE
# Integrated/Standalone → Validation → AI Review → Mapping →
# Staging → Governed Promotion → Existing Price Engine
# ══════════════════════════════════════════════════════════════════════

_OB_STG = "PRICING_ENGINE_DB.CORE_STAGING"

_OB_TARGET_FIELDS = {
    "CUSTOMER_ID": {
        "required": True,
        "datatype": "VARCHAR",
        "aliases": ["customer_id", "customerid", "customer_code", "customercode"],
    },
    "CUSTOMER_NAME": {
        "required": False,
        "datatype": "VARCHAR",
        "aliases": ["customer_name", "customername", "customer"],
    },
    "SKU": {
        "required": True,
        "datatype": "VARCHAR",
        "aliases": ["sku", "product_id", "productid", "material_number", "materialnumber"],
    },
    "PRODUCT_NAME": {
        "required": False,
        "datatype": "VARCHAR",
        "aliases": ["product_name", "productname", "description", "product_description"],
    },
    "PRODUCT_CATEGORY": {
        "required": False,
        "datatype": "VARCHAR",
        "aliases": ["product_category", "category", "product_family"],
    },
    "PLANT": {
        "required": False,
        "datatype": "VARCHAR",
        "aliases": ["plant", "plant_id", "plant_name", "factory"],
    },
    "PRODUCT_COST": {
        "required": True,
        "datatype": "NUMBER",
        "aliases": [
            "product_cost", "total_cost", "standard_cost",
            "std_cost", "cost", "transfer_price"
        ],
    },
    "SELLING_PRICE": {
        "required": False,
        "datatype": "NUMBER",
        "aliases": ["selling_price", "net_price", "price", "target_price"],
    },
    "TARGET_MARGIN": {
        "required": False,
        "datatype": "NUMBER",
        "aliases": ["target_margin", "margin", "margin_percent", "margin_pct"],
    },
    "DISCOUNT_PERCENT": {
        "required": False,
        "datatype": "NUMBER",
        "aliases": ["discount", "discount_percent", "discount_pct"],
    },
    "FREIGHT": {
        "required": False,
        "datatype": "NUMBER",
        "aliases": ["freight", "freight_cost"],
    },
    "PACKAGING": {
        "required": False,
        "datatype": "NUMBER",
        "aliases": ["packaging", "packaging_cost"],
    },
    "TAX_PERCENT": {
        "required": False,
        "datatype": "NUMBER",
        "aliases": ["tax", "taxes", "tax_percent", "tax_pct"],
    },
    "OTHER_CHARGES": {
        "required": False,
        "datatype": "NUMBER",
        "aliases": ["other_charges", "other_cost", "surcharge"],
    },
    "CURRENCY": {
        "required": True,
        "datatype": "VARCHAR",
        "aliases": ["currency", "currency_code", "currencycode"],
    },
    "CUSTOMER_SEGMENT": {
        "required": False,
        "datatype": "VARCHAR",
        "aliases": ["customer_segment", "segment", "customer_group"],
    },
    "SALES_CHANNEL": {
        "required": False,
        "datatype": "VARCHAR",
        "aliases": ["sales_channel", "channel"],
    },
    "EFFECTIVE_DATE": {
        "required": False,
        "datatype": "DATE",
        "aliases": ["effective_date", "valid_from", "effectivefrom", "date"],
    },
}


def ob_normalize_column_name(name):
    """Normalize source names only for alias matching."""
    return re.sub(r"[^a-z0-9]+", "_", str(name).strip().lower()).strip("_")


def ob_file_hash(file_bytes):
    return hashlib.sha256(file_bytes).hexdigest()


def ob_dataframe_hash(df):
...
def ob_safe_text(value, max_length=500):
...
def ob_read_uploaded_file(uploaded_file, selected_sheet=None):
    """
    Reads CSV/XLSX into memory.
    No Snowflake write occurs here.
    Returns (DataFrame, metadata, error).
    """
    ...
    if extension not in ("csv", "xlsx"):
        return None, None, "Only CSV and XLSX files are supported."
    ...
    # Governed application limit, not a Snowflake platform limit.
    max_bytes = 25 * 1024 * 1024
    if len(file_bytes) > max_bytes:
        return None, None, "File exceeds the configured 25 MB onboarding limit."
    ...
    try:
        if extension == "csv":
            ...
            if not sheet_names:
                return None, None, "The workbook contains no readable sheets."

            sheet_name = selected_sheet or sheet_names[0]
            if sheet_name not in sheet_names:
                return None, None, "The selected worksheet is not available."
        ...
        if df is None or df.empty:
            return None, None, "The selected file or worksheet contains no data rows."

        if len(df.columns) == 0:
            return None, None, "No headers were detected."
        ...
        metadata = {
            ...
            "source_hash": ob_file_hash(file_bytes),
            "sheet_name": sheet_name,
        ...
        return None, None, (
            "XLSX support requires the openpyxl package in the "
            "Streamlit-in-Snowflake environment."
        )
    except Exception as exc:
        return None, None, f"Could not read the uploaded file: {type(exc).__name__}: {exc}"


def ob_create_batch(workspace_id, mode, metadata, user):
    batch_id = _ob_id("BAT")

    duplicate = _wf_df(f"""
        SELECT BATCH_ID, BATCH_STATUS
        FROM {_OB}.ONBOARDING_BATCHES
        WHERE WORKSPACE_ID = '{_sql_escape(workspace_id)}'
          AND SOURCE_HASH = '{_sql_escape(metadata.get("source_hash"))}'
          AND BATCH_STATUS = 'PROMOTED'
        LIMIT 1
    """)

    if not duplicate.empty:
        return None, (
            "This file or payload was already promoted under batch "
            f"{duplicate.iloc[0]['BATCH_ID']}."
        )

    metadata_json = _sql_escape(json.dumps(metadata, default=str))

    ok, err = _wf_exec(f"""
        INSERT INTO {_OB}.ONBOARDING_BATCHES (
            BATCH_ID,
            WORKSPACE_ID,
            SOURCE_MODE,
            SOURCE_TYPE,
        ...
        SELECT
            '{_sql_escape(batch_id)}',
            '{_sql_escape(workspace_id)}',
            '{_sql_escape(mode)}',
            '{_sql_escape(metadata.get("source_type"))}',
            '{_sql_escape(metadata.get("file_name") or metadata.get("source_name") or "")}',
            '{_sql_escape(metadata.get("source_hash") or "")}',
            '{_sql_escape(metadata.get("sheet_name") or "")}',
            {int(metadata.get("row_count") or 0)},
            {int(metadata.get("column_count") or 0)},
            'UPLOADED',
            0,
            0,
            0,
        ...
        WHERE NOT EXISTS (
            SELECT 1
            FROM {_OB}.ONBOARDING_BATCHES
            WHERE BATCH_ID = '{_sql_escape(batch_id)}'
        )
    """)
    ...
    ob_log(
        workspace_id,
        "DATA_BATCH_CREATED",
        f"Batch {batch_id}; source={metadata.get('source_type')}",
        user,
    )
...
def ob_suggest_column_mapping(df):
    """
    Deterministic alias-based suggestions.
    Cortex is not required to map columns.
    """
    suggestions = {}
    source_columns = list(df.columns)

    for source_column in source_columns:
        normalized_source = ob_normalize_column_name(source_column)
        target = None

        for target_column, definition in _OB_TARGET_FIELDS.items():
            candidates = [
                ob_normalize_column_name(target_column),
                *[ob_normalize_column_name(x) for x in definition["aliases"]],
            ]
            if normalized_source in candidates:
                target = target_column
                break

        suggestions[source_column] = target or "DO_NOT_IMPORT"

    return suggestions


def ob_validate_dataframe(df, mapping):
    """
    Deterministic validation only.
    Returns findings, normalized preview, and a summary.
    """
    findings = []
    ...
    mapped_targets = {
        ...
        if target and target != "DO_NOT_IMPORT"
    }

    required_targets = {
        target for target, definition in _OB_TARGET_FIELDS.items()
        if definition["required"]
    }

    for missing_target in sorted(required_targets - mapped_targets):
        findings.append({
            "row_number": None,
            "column_name": missing_target,
            "rule_code": "REQUIRED_MAPPING_MISSING",
            "severity": "ERROR",
            "message": f"Required target field {missing_target} is not mapped.",
            "invalid_value": None,
        })
    ...
    for source_column in source_columns:
        selected_target = mapping[source_column]

        if selected_target == "DO_NOT_IMPORT":
            continue

        duplicate_sources = [
            c for c, target in mapping.items()
            if target == selected_target and c != source_column
        ]
        ...
        if duplicate_sources:
            findings.append({
                ...
                "rule_code": "DUPLICATE_TARGET_MAPPING",
                "severity": "ERROR",
                "message": (
                    f"Multiple source columns map to {selected_target}: "
                    f"{source_column}, {', '.join(duplicate_sources)}"
                ),
                "invalid_value": None,
            })
    ...
    for source_column, target_column in mapping.items():
        if target_column == "DO_NOT_IMPORT":
            continue

        definition = _OB_TARGET_FIELDS[target_column]
        datatype = definition["datatype"]

        if datatype == "NUMBER":
            normalized[target_column] = df[source_column].apply(ob_safe_number)
        elif datatype == "DATE":
            normalized[target_column] = df[source_column].apply(ob_safe_date)
        else:
            normalized[target_column] = df[source_column].apply(ob_safe_text)

    # Ensure every canonical column exists.
    for target_column in _OB_TARGET_FIELDS:
    ...
    for row_index, row in normalized.iterrows():
        ...
        for target_column in required_targets:
            ...
            if value is None or pd.isna(value) or str(value).strip() == "":
                findings.append({
                    ...
                    "severity": "ERROR",
                    "message": f"{target_column} is required.",
                    "invalid_value": None,
                })
        ...
        for field in ["TARGET_MARGIN", "DISCOUNT_PERCENT", "TAX_PERCENT"]:
            value = row.get(field)
            if value is not None and not pd.isna(value):
                ...
                if number < 0 or number > 100:
                    findings.append({
                        "row_number": source_row,
                        "column_name": field,
                        "rule_code": "PERCENT_OUT_OF_RANGE",
                        "severity": "ERROR",
        ...
        if currency and currency not in {"INR", "USD", "EUR", "GBP", "JPY"}:
            findings.append({
                "row_number": source_row,
                "column_name": "CURRENCY",
                "rule_code": "UNSUPPORTED_CURRENCY",
                "severity": "WARNING",
                "message": f"Currency {currency} is not in the configured currency list.",
                "invalid_value": str(currency),
            })
            warning_rows.add(row_index)
    ...
    duplicate_subset = [
        ...
        if c in normalized.columns and normalized[c].notna().any()
    ]
    ...
    if len(duplicate_subset) >= 2:
        ...
        for row_index in normalized.index[duplicate_mask]:
            findings.append({
                "row_number": int(row_index) + 2,
                "column_name": ",".join(duplicate_subset),
                "rule_code": "POTENTIAL_DUPLICATE",
                "severity": "WARNING",
                "message": "Potential duplicate business record.",
                "invalid_value": None,
            })
            warning_rows.add(row_index)
    ...
    score = round(
        (valid_rows / total_rows * 100) if total_rows else 0,
        2,
    )
    ...
    normalized["VALIDATION_STATUS"] = [
        "INVALID" if idx in error_rows
        else "VALID_WITH_WARNINGS" if idx in warning_rows
        else "VALID"
        for idx in normalized.index
    ]

    normalized["SOURCE_ROW_NUMBER"] = [
        int(idx) + 2 for idx in normalized.index
    ]
...
def ob_persist_validation(batch_id, workspace_id, findings, summary):
    _wf_exec(f"""
        DELETE FROM {_OB}.DATA_VALIDATION_RESULTS
        WHERE BATCH_ID = '{_sql_escape(batch_id)}'
    """)
    ...
    for finding in findings:
        row_number_sql = (
            ...
            else str(int(finding["row_number"]))
        )

        invalid_value_sql = (
            "NULL"
            if finding["invalid_value"] is None
            else f"'{_sql_escape(finding['invalid_value'])}'"
        )

        ok, err = _wf_exec(f"""
            INSERT INTO {_OB}.DATA_VALIDATION_RESULTS (
                VALIDATION_ID,
                BATCH_ID,
                WORKSPACE_ID,
                ROW_NUMBER,
            ...
            VALUES (
                ...
                '{_sql_escape(workspace_id)}',
                {row_number_sql},
                '{_sql_escape(finding.get("column_name") or "")}',
                '{_sql_escape(finding.get("rule_code") or "")}',
                '{_sql_escape(finding.get("severity") or "")}',
                '{_sql_escape(finding.get("message") or "")}',
                {invalid_value_sql},
                CURRENT_TIMESTAMP()
            )
        """)
    ...
    batch_status = (
        "VALIDATED"
        if summary["status"] in ("VALID", "VALID_WITH_WARNINGS")
        else "BLOCKED"
    )

    ok, err = _wf_exec(f"""
        UPDATE {_OB}.ONBOARDING_BATCHES
        SET
            BATCH_STATUS = '{batch_status}',
            VALID_ROWS = {int(summary["valid_rows"])},
            INVALID_ROWS = {int(summary["invalid_rows"])},
            WARNING_ROWS = {int(summary["warning_rows"])},
            VALIDATION_SCORE = {float(summary["score"])},
...
def ob_save_mapping(batch_id, workspace_id, mapping, df, user):
    _wf_exec(f"""
        DELETE FROM {_OB}.COLUMN_MAPPING_CONFIG
        WHERE BATCH_ID = '{_sql_escape(batch_id)}'
    """)
    ...
    for source_column, target_column in mapping.items():
        ...
        target_definition = _OB_TARGET_FIELDS[target_column]
        source_dtype = str(df[source_column].dtype)

        ok, err = _wf_exec(f"""
            INSERT INTO {_OB}.COLUMN_MAPPING_CONFIG (
                MAPPING_ID,
                WORKSPACE_ID,
                BATCH_ID,
                SOURCE_NAME,
            ...
            VALUES (
                ...
                '{_sql_escape(workspace_id)}',
                '{_sql_escape(batch_id)}',
                '{_sql_escape(st.session_state.get("onboarding_source_name") or "")}',
                '{_sql_escape(source_column)}',
                '{_sql_escape(target_column)}',
                '{_sql_escape(source_dtype)}',
                '{_sql_escape(target_definition["datatype"])}',
                {str(bool(target_definition["required"])).upper()},
                'CONFIRMED',
                100,
                '{_sql_escape(user)}',
                CURRENT_TIMESTAMP(),
...
def ob_ai_data_quality_review(batch_id, normalized_df, findings, summary):
    """
    Advisory only.
    No data modification, blocking, staging, or promotion.
    """
    numeric_summary = {}
    ...
    finding_counts = {}
    for finding in findings:
        key = f"{finding['severity']}:{finding['rule_code']}"
        finding_counts[key] = finding_counts.get(key, 0) + 1

    facts = {
        "batch_id": batch_id,
        "validation_summary": summary,
        "deterministic_finding_counts": finding_counts,
    ...
    return ai_reason_score(
        ...
        decision_type="data_quality_review",
        context_facts=json.dumps(facts, default=str),
        extra_instruction=(
            "Review only the supplied deterministic profile. "
            "Identify suspicious distributions, possible unit or currency issues, "
            "unusual cost-price relationships, potential mapping concerns, "
            "missing information and suggested operator questions. "
            "Do not invent row values. Do not modify data. "
            "The score represents overall data quality readiness."
        ),
        cache_key=_ai_cache_key(
            "ob_data_quality",
            batch_id,
            json.dumps(summary, sort_keys=True),
            json.dumps(numeric_summary, sort_keys=True),
        ),
    )


def ob_stage_normalized_data(
    batch_id,
    workspace_id,
    normalized_df,
    source_mode,
    ...
    """
    Writes only normalized valid rows to controlled staging.
    Invalid rows remain in validation history and are not staged.
    """
    if normalized_df is None or normalized_df.empty:
        return False, "No normalized rows are available."

    valid_df = normalized_df[
        normalized_df["VALIDATION_STATUS"].isin(
            ["VALID", "VALID_WITH_WARNINGS"]
        )
    ].copy()

    if valid_df.empty:
        return False, "No valid rows are available for staging."

    # Duplicate-stage protection.
    existing = _wf_df(f"""
        SELECT COUNT(*) AS ROW_COUNT
        FROM {_OB_STG}.STG_PRICING_INPUTS
        WHERE BATCH_ID = '{_sql_escape(batch_id)}'
    """)

    if not existing.empty and int(existing.iloc[0]["ROW_COUNT"]) > 0:
        return False, "This batch has already been written to staging."
    ...
    for _, row in valid_df.iterrows():
        ...
        business_payload = {
            ...
            for field in _OB_TARGET_FIELDS
        }
        ...
        stage_rows.append({
            "STAGING_ID": _ob_id("STG"),
            "BATCH_ID": batch_id,
            "WORKSPACE_ID": workspace_id,
            "CUSTOMER_ID": ob_safe_text(row.get("CUSTOMER_ID")),
            "CUSTOMER_NAME": ob_safe_text(row.get("CUSTOMER_NAME")),
            "SKU": ob_safe_text(row.get("SKU")),
            "PRODUCT_NAME": ob_safe_text(row.get("PRODUCT_NAME")),
            "PRODUCT_CATEGORY": ob_safe_text(row.get("PRODUCT_CATEGORY")),
            "PLANT": ob_safe_text(row.get("PLANT")),
            "PRODUCT_COST": ob_safe_number(row.get("PRODUCT_COST")),
            "SELLING_PRICE": ob_safe_number(row.get("SELLING_PRICE")),
            "TARGET_MARGIN": ob_safe_number(row.get("TARGET_MARGIN")),
            "DISCOUNT_PERCENT": ob_safe_number(row.get("DISCOUNT_PERCENT")),
            "FREIGHT": ob_safe_number(row.get("FREIGHT")),
            "PACKAGING": ob_safe_number(row.get("PACKAGING")),
            "TAX_PERCENT": ob_safe_number(row.get("TAX_PERCENT")),
            "OTHER_CHARGES": ob_safe_number(row.get("OTHER_CHARGES")),
            "CURRENCY": ob_safe_text(row.get("CURRENCY")),
            "CUSTOMER_SEGMENT": ob_safe_text(row.get("CUSTOMER_SEGMENT")),
            "SALES_CHANNEL": ob_safe_text(row.get("SALES_CHANNEL")),
            "EFFECTIVE_DATE": ob_safe_date(row.get("EFFECTIVE_DATE")),
            "SOURCE_SYSTEM": ob_safe_text(source_system),
            "SOURCE_ROW_NUMBER": source_row,
            "ROW_HASH": row_hash,
            "VALIDATION_STATUS": ob_safe_text(row.get("VALIDATION_STATUS")),
            "CREATED_BY": ob_safe_text(user),

            # Required because STG_PRICING_INPUTS has 27 target columns.
            # Use a timezone-naive timestamp for TIMESTAMP_NTZ.
    ...
    try:
        ...
        staging_pdf = pd.DataFrame(stage_rows)

        missing_columns = [
            column
            for column in staging_columns
            if column not in staging_pdf.columns
        ]

        if missing_columns:
            return False, (
                "Staging preparation failed. Missing source columns: "
                + ", ".join(missing_columns)
            )

        # Keep the Snowpark DataFrame in exactly the same order as the
        # existing Snowflake staging table.
        staging_pdf = staging_pdf[staging_columns]

        snowpark_df = session.create_dataframe(staging_pdf)
        

        snowpark_df.write.mode("append").save_as_table(
            f"{_OB_STG}.STG_PRICING_INPUTS"
        )
        ...
        ob_log(
            workspace_id,
            "STAGING_COMPLETED",
            f"Batch={batch_id}; rows={len(stage_rows)}",
            user,
        )

        return True, f"{len(stage_rows)} row(s) written to staging."
        ...
        _wf_exec(f"""
            ...
            SET
                ...
                BATCH_STATUS = 'FAILED',
                ERROR_MESSAGE = '{_sql_escape(str(exc))}',
                UPDATED_AT = CURRENT_TIMESTAMP()
            WHERE BATCH_ID = '{_sql_escape(batch_id)}'
        """)
...
def ob_promote_batch(batch_id, workspace_id, source_mode, user):
    """
    Controlled staging-to-production promotion.

    - Reloads persisted batch state.
    - Prevents repeated promotion.
    - Promotes only valid staged rows.
    - Uses a Snowflake transaction.
    - AI is not involved.
    """
    batch_df = _wf_df(f"""
        SELECT *
        FROM {_OB}.ONBOARDING_BATCHES
        WHERE BATCH_ID = '{_sql_escape(batch_id)}'
          AND WORKSPACE_ID = '{_sql_escape(workspace_id)}'
        LIMIT 1
    """)
    ...
    batch = batch_df.iloc[0].to_dict()

    if batch.get("PROMOTION_STATUS") == "COMPLETED":
        return False, (
            "This onboarding batch has already been promoted. "
            "Refresh to view the current workspace status."
        )

    if batch.get("STAGING_STATUS") != "COMPLETED":
        return False, "Staging must complete before promotion."

    if int(batch.get("INVALID_ROWS") or 0) > 0:
        return False, "Blocking validation errors must be resolved before promotion."

    staged_count_df = _wf_df(f"""
        SELECT COUNT(*) AS ROW_COUNT
        FROM {_OB_STG}.STG_PRICING_INPUTS
        WHERE BATCH_ID = '{_sql_escape(batch_id)}'
          AND WORKSPACE_ID = '{_sql_escape(workspace_id)}'
    ...
    staged_count = (
        int(staged_count_df.iloc[0]["ROW_COUNT"])
        if not staged_count_df.empty
        else 0
    )

    if staged_count == 0:
        return False, "No valid staged records exist for this batch."

    promotion_id = _ob_id("PRM")

    try:
        session.sql("BEGIN").collect()

        session.sql(f"""
            INSERT INTO {_OB}.DATA_PROMOTION_LOG (
                PROMOTION_ID,
                BATCH_ID,
                WORKSPACE_ID,
                SOURCE_TABLE,
        ...
        # Deactivate prior workspace/customer/SKU onboarding input.
        session.sql(f"""
            UPDATE PRICING_ENGINE_DB.CORE_INPUT.ONBOARDED_PRICING_INPUTS target
            SET
                IS_ACTIVE = FALSE,
                UPDATED_AT = CURRENT_TIMESTAMP()
            FROM {_OB_STG}.STG_PRICING_INPUTS source
            WHERE source.BATCH_ID = '{_sql_escape(batch_id)}'
              AND source.WORKSPACE_ID = '{_sql_escape(workspace_id)}'
              AND target.WORKSPACE_ID = source.WORKSPACE_ID
              AND COALESCE(target.CUSTOMER_ID, '') = COALESCE(source.CUSTOMER_ID, '')
              AND COALESCE(target.SKU, '') = COALESCE(source.SKU, '')
              AND target.IS_ACTIVE = TRUE
        """).collect()

        session.sql(f"""
            INSERT INTO PRICING_ENGINE_DB.CORE_INPUT.ONBOARDED_PRICING_INPUTS (
                PRICING_INPUT_ID,
                WORKSPACE_ID,
                BATCH_ID,
                CUSTOMER_ID,
        ...
        session.sql(f"""
            UPDATE {_OB}.DATA_PROMOTION_LOG
            SET
                PROMOTED_ROWS = {staged_count},
                REJECTED_ROWS = 0,
                PROMOTION_STATUS = 'COMPLETED',
        ...
        ob_log(
            ...
            f"Batch={batch_id}; rows={staged_count}",
            user,
        )

        return True, f"{staged_count} row(s) promoted successfully."
        ...
        _wf_exec(f"""
            ...
            VALUES (
                ...
                CURRENT_TIMESTAMP(),
                CURRENT_TIMESTAMP(),
                '{_sql_escape(error_text)}'
            )
        """)

        return False, f"Promotion failed and was rolled back: {error_text}"


def ob_reset_data_pipeline():
    for key, default in {
        "onboarding_batch_id": None,
        "onboarding_source_df": None,
        "onboarding_source_name": None,
        "onboarding_source_type": None,
        "onboarding_source_hash": None,
        "onboarding_sheet_name": None,
        "onboarding_validation_results": [],
        "onboarding_validation_summary": None,
        "onboarding_column_mapping": {},
        "onboarding_mapping_confirmed": False,
        "onboarding_staging_complete": False,
        "onboarding_promotion_complete": False,
        "onboarding_ai_quality_result": None,
    }.items():
        st.session_state[key] = default
...
#
# This runs BEFORE the main_nav_tab widget is instantiated.
# A button elsewhere sets pending_main_nav_tab and calls st.rerun().
# On the next run, the pending destination is applied safely here.
# ─────────────────────────────────────────────────────────────
_pending_nav = st.session_state.get("pending_main_nav_tab")

if _pending_nav:
    if _pending_nav in _TAB_LABELS:
        st.session_state["main_nav_tab"] = _pending_nav

    # Clear it immediately so later reruns do not repeatedly force the tab.
    st.session_state["pending_main_nav_tab"] = None

# Protect against a stale/renamed navigation value.
if (
    st.session_state.get("main_nav_tab") is not None
    and st.session_state.get("main_nav_tab") not in _TAB_LABELS
):
    st.session_state["main_nav_tab"] = _TAB_LABELS[0]
...
# ══════════════════════════════════════════════════════════════════════
# TAB 0 — 🏢 CUSTOMER ONBOARDING  (enterprise SaaS-style onboarding)
# First step before entering the platform. Two modes: Integrated / Standalone.
# NO Cost Model is built or simulated. Integrated mode is INTEGRATION-READY.
# ══════════════════════════════════════════════════════════════════════
if _active_tab == "🏢 Customer Onboarding":
    _ob_user = get_current_user()
    section("🏢", "Customer Onboarding")
    st.markdown('<div style="opacity:.75;margin-top:-6px">Onboard a customer once — '
                'choose how you use the platform, initialize a governed workspace, then '
                'proceed into the Pricing Engine.</div>', unsafe_allow_html=True)
    divider()

    _mode = st.session_state.get("selected_onboarding_mode")

    # ─────────────────── FIRST SCREEN: MODE SELECTION ───────────────────
    if not _mode:
        st.markdown("### How would you like to use the Pricing Intelligence Platform?")
        oc1, oc2 = st.columns(2)
        _card = ("border:1px solid rgba(255,255,255,.10);border-radius:14px;"
                 "padding:22px;background:rgba(255,255,255,.03);min-height:270px")
        with oc1:
            st.markdown(f"""<div style="{_card};border-left:4px solid #00C7B2">
                <div style="font-size:34px">🏭</div>
                <h3 style="margin:.3rem 0;color:#E6EDF3">Integrated Pricing Platform</h3>
                <p style="color:#9FB3C8;font-size:13px">Choose this if your organization uses
                <b>both</b> the Manufacturing Cost Model and the Pricing Intelligence Platform.
                The Cost Model automatically provides manufacturing costs to the Pricing Platform
                through enterprise integration. Suitable for the complete manufacturing ecosystem.</p>
                </div>""", unsafe_allow_html=True)
            if st.button("Continue with Integrated Mode", key="ob_pick_integrated",
                         use_container_width=True):
                st.session_state["selected_onboarding_mode"] = "INTEGRATED"
                ob_record_mode(st.session_state.get("workspace_id"), "INTEGRATED", _ob_user)
                st.rerun()
        with oc2:
            st.markdown(f"""<div style="{_card};border-left:4px solid #38BDF8">
                <div style="font-size:34px">💰</div>
                <h3 style="margin:.3rem 0;color:#E6EDF3">Standalone Pricing Platform</h3>
                <p style="color:#9FB3C8;font-size:13px">Choose this if you only want to use the
                Pricing Intelligence Platform. No Manufacturing Cost Model is required — you will
                manually provide pricing inputs before generating quotations.</p>
                </div>""", unsafe_allow_html=True)
            if st.button("Continue with Standalone Mode", key="ob_pick_standalone",
                         use_container_width=True):
                st.session_state["selected_onboarding_mode"] = "STANDALONE"
                ob_record_mode(st.session_state.get("workspace_id"), "STANDALONE", _ob_user)
                st.rerun()

    # ─────────────────── ONBOARDING WORKFLOW (mode chosen) ───────────────────
    else:
        top1, top2 = st.columns([3, 1])
        with top1:
            st.caption(f"Mode: **{_mode}**  ·  Signed in as **{_ob_user}**  ·  "
                       f"Workspace: **{st.session_state.get('workspace_id') or 'not initialized'}**")
        with top2:
            if st.button("↩ Change Mode", key="ob_change_mode", use_container_width=True):
                st.session_state["selected_onboarding_mode"] = None
                st.rerun()
        ...
        step = st.radio("Onboarding step", steps, horizontal=True,
                        label_visibility="collapsed", key="ob_step_nav")
        divider()
        info = st.session_state["onboarding_customer_info"]
        ...
        if step == "🧾 Customer Info":
            ...
            c1, c2, c3 = st.columns(3)
            with c1:
                info["customer_name"] = st.text_input("Customer Name", info.get("customer_name",""))
                # Real customer IDs from existing tables (keeps Price Engine prefill valid)
                cust_ids = _wf_df("SELECT DISTINCT CUSTOMER_ID FROM "
                                  "PRICING_ENGINE_DB.CORE_INPUT.CUSTOMER_AGREEMENTS ORDER BY CUSTOMER_ID")
                cid_opts = ["(new / manual)"] + (cust_ids["CUSTOMER_ID"].tolist() if not cust_ids.empty else [])
                pick_cid = st.selectbox("Customer ID (from Snowflake)", cid_opts,
                                        index=cid_opts.index(info["customer_id"])
                                        if info.get("customer_id") in cid_opts else 0)
                info["customer_id"] = "" if pick_cid == "(new / manual)" else pick_cid
                info["industry"] = st.text_input("Industry", info.get("industry",""))
                info["region"] = st.text_input("Region", info.get("region",""))
            with c2:
                info["business_unit"] = st.text_input("Business Unit", info.get("business_unit",""))
                info["country"] = st.text_input("Country", info.get("country",""))
                info["sales_rep"] = st.text_input("Sales Representative", info.get("sales_rep",""))
                info["currency"] = st.selectbox("Currency", ["INR","USD","EUR","GBP","JPY"],
                                                index=["INR","USD","EUR","GBP","JPY"].index(info.get("currency","INR")))
            with c3:
                info["language"] = st.selectbox("Language", ["English","Hindi","German","French","Japanese"],
                                                index=["English","Hindi","German","French","Japanese"].index(info.get("language","English")))
                info["customer_type"] = st.selectbox("Customer Type", ["Enterprise","Mid-Market","SMB","OEM","Distributor"],
                                                      index=["Enterprise","Mid-Market","SMB","OEM","Distributor"].index(info.get("customer_type","Enterprise")))
                info["workspace_name"] = st.text_input("Workspace Name",
                                                       info.get("workspace_name","") or (f"{info.get('customer_name','')} Workspace".strip()))
                st.text_input("Created By", _ob_user, disabled=True)
                st.text_input("Created Date", pd.Timestamp.utcnow().strftime("%Y-%m-%d"), disabled=True)
            st.session_state["onboarding_customer_info"] = info
            st.session_state["onboarding_selected_customer"] = info.get("customer_id") or None
            if info.get("customer_name"):
                st.success("Customer information captured. Proceed to **🗂️ Workspace**.")

        # ---- 2. WORKSPACE INITIALIZATION ----
        elif step == "🗂️ Workspace":
            section("🗂️", "Workspace Initialization")
            w1, w2, w3 = st.columns(3)
            w1.metric("Workspace Status",
                      "INITIALIZED" if st.session_state.get("workspace_id") else "NOT INITIALIZED")
            w2.metric("Workspace ID", st.session_state.get("workspace_id") or "—")
            w3.metric("Mode", _mode)
            b1, b2 = st.columns(2)
            with b1:
                if st.button("🚀 Initialize Workspace", use_container_width=True, key="ob_init_ws"):
                    if not info.get("customer_name"):
                        st.error("Complete Customer Information first.")
                    else:
                        wid, err = ob_init_workspace(info, _mode, _ob_user)
                        if wid:
                            st.session_state["workspace_id"] = wid
                            st.success(f"✅ Workspace initialized: **{wid}**")
                        else:
                            st.error(f"Initialization failed (no fake success): {err}")
            with b2:
                if st.button("♻️ Reset Workspace", use_container_width=True, key="ob_reset_ws"):
                    wid = st.session_state.get("workspace_id")
                    if wid:
                        _wf_exec(f"UPDATE {_OB}.CUSTOMER_WORKSPACES SET WORKSPACE_STATUS='ARCHIVED', "
                                 f"UPDATED_AT=CURRENT_TIMESTAMP() WHERE WORKSPACE_ID='{_sql_escape(wid)}'")
                        ob_log(wid, "WORKSPACE_RESET", "User reset workspace", _ob_user)
                    for k in ["workspace_id","onboarding_selected_product","integration_ready",
                              "onboarding_completed","standalone_pricing_inputs"]:
                        st.session_state[k] = None if k != "integration_ready" else False
                    st.success("Workspace reset. Re-initialize to continue.")

        # ---- 3. SNOWFLAKE VALIDATION (never faked) ----
        elif step == "❄️ Snowflake Validation":
            section("❄️", "Snowflake Validation")
            checks = {
                "Database / Warehouse (session)": ("connected" if _ob_user != "UNKNOWN" else "missing"),
                "Customer Tables (CUSTOMER_AGREEMENTS)": ob_validate_object("PRICING_ENGINE_DB.CORE_INPUT.CUSTOMER_AGREEMENTS"),
                "Product Tables (CALCULATED_STANDARD_COSTS)": ob_validate_object("PRICING_ENGINE_DB.CORE_INPUT.CALCULATED_STANDARD_COSTS"),
                "Pricing Tables (CUSTOMER_PRICING)": ob_validate_object("PRICING_ENGINE_DB.CORE_OUTPUT.CUSTOMER_PRICING"),
                "Pricing Matrix (OPTIMIZED_PRICING_MATRIX)": ob_validate_object("PRICING_ENGINE_DB.CORE_OUTPUT.OPTIMIZED_PRICING_MATRIX"),
                "Price Sensitivity": ob_validate_object("PRICING_ENGINE_DB.CORE_INPUT.PRICE_SENSITIVITY"),
                "Plant Capacity": ob_validate_object("PRICING_ENGINE_DB.CORE_INPUT.PLANT_CAPACITY"),
                "Workspace Table": ob_validate_object(f"{_OB}.CUSTOMER_WORKSPACES"),
            }
            cortex_ok = ob_cortex_available()
            rows = []
            for name, state in checks.items():
                icon = "✅ Connected" if state == "connected" else "❌ Missing"
                rows.append({"Object": name, "Status": icon})
            rows.append({"Object": "Cortex AI Availability",
                         "Status": "✅ Connected" if cortex_ok else "⚠️ Warning — Cortex not responding"})
            st.dataframe(pd.DataFrame(rows), use_container_width=True, hide_index=True)
            st.session_state["_ob_sf_ok"] = all(v == "connected" for v in checks.values())
            if st.session_state["_ob_sf_ok"]:
                st.success("Core Snowflake objects reachable.")
            else:
                st.warning("Some objects are missing — resolve before proceeding for full functionality.")

                # ============================================================
        # DATA SOURCE
        # ============================================================
        elif step == "📥 Data Source":
            section("📥", "Data Source")

            workspace_id = st.session_state.get("workspace_id")

            if not workspace_id:
                st.warning("Initialize the customer workspace before loading data.")

            elif _mode == "INTEGRATED":
                section("🔗", "External Cost Model Integration")

                cfg = ob_integration_status()

                if not cfg or not cfg.get("IS_CONFIGURED"):
                    st.warning(
                        "Manufacturing Cost Model integration has not been configured."
                    )

                    st.session_state["integration_ready"] = False

                    integration_rows = [
                        {"Check": "Cost Model Connection", "Status": "❌ Missing"},
                        {"Check": "Shared Customer IDs", "Status": "❌ Missing"},
                        {"Check": "Shared Product IDs", "Status": "❌ Missing"},
                        {"Check": "Shared Cost Tables", "Status": "❌ Missing"},
                        {"Check": "Authentication", "Status": "❌ Missing"},
                        {"Check": "API Endpoint", "Status": "❌ Missing"},
                        {"Check": "Data Synchronization", "Status": "❌ Missing"},
                    ]
                    ...
                    st.info(
                        "No manufacturing-cost data has been created or simulated. "
                        "Configure the external integration or return to mode selection "
                        "and choose Standalone Mode."
                    )

                else:
                    st.session_state["integration_ready"] = True
                    st.success("Cost Model integration configuration detected.")
                    ...
                    ic2.metric(
                        "Authentication",
                        cfg.get("AUTH_STATUS") or "Not Configured",
                    )
                    ic3.metric(
                        "Last Synchronization",
                        str(cfg.get("LAST_SYNC_AT") or "No synchronization recorded"),
                    )

                    st.caption(
                        "Imported manufacturing fields remain read-only and are owned "
                        "by the external Cost Model."
                    )

                    # No fabricated import implementation.
                    st.info(
                        "The integration configuration exists, but this app will only "
                        "load data after the external Cost Model supplies a real shared "
                        "view/table or API payload."
                    )

            else:
                section("📤", "Upload Pricing Data")

                st.caption(
                    "Upload CSV or XLSX. The file remains outside production tables "
                    "until validation, mapping, staging, and promotion complete."
                )

                uploaded_file = st.file_uploader(
                    "Upload CSV/XLSX",
                    type=["csv", "xlsx"],
                    key="ob_source_upload",
                )
                ...
                if st.button(
                    ...
                    df, metadata, error = ob_read_uploaded_file(
                        uploaded_file,
                        selected_sheet,
                    )
                    ...
                    if error:
                        ...
                        batch_id, batch_error = ob_create_batch(
                            workspace_id,
                            _mode,
                            metadata,
                            _ob_user,
                        ...
                        if batch_error:
                            ...
                            st.session_state["onboarding_batch_id"] = batch_id
                            st.session_state["onboarding_source_df"] = df
                            st.session_state["onboarding_source_name"] = metadata["file_name"]
                            st.session_state["onboarding_source_type"] = metadata["source_type"]
                            st.session_state["onboarding_source_hash"] = metadata["source_hash"]
                            st.session_state["onboarding_sheet_name"] = metadata["sheet_name"]

                            suggested_mapping = ob_suggest_column_mapping(df)
                            st.session_state["onboarding_column_mapping"] = suggested_mapping

                            st.success(
                                f"Loaded {metadata['row_count']} row(s) and "
                                f"{metadata['column_count']} column(s). "
                                f"Batch ID: {batch_id}"
                            )

                source_df = st.session_state.get("onboarding_source_df")

                if source_df is not None:
                    m1, m2, m3, m4 = st.columns(4)
                    m1.metric(
                        "File",
                        st.session_state.get("onboarding_source_name") or "—",
                    )
                    m2.metric("Rows", len(source_df))
                    m3.metric("Columns", len(source_df.columns))
                    m4.metric(
                        "Batch",
                        st.session_state.get("onboarding_batch_id") or "—",
                    )
        ...
        # DATA PREVIEW
        # ============================================================
        elif step == "👁️ Data Preview":
            section("👁️", "Source Data Preview")

            source_df = st.session_state.get("onboarding_source_df")

            if source_df is None:
                st.info("Load a Standalone file or Integrated payload first.")

            else:
                p1, p2, p3 = st.columns(3)
                p1.metric("Rows", len(source_df))
                p2.metric("Columns", len(source_df.columns))
                p3.metric(
                    "Source",
                    st.session_state.get("onboarding_source_type") or "—",
                )
                ...
                with st.expander("Detected Columns"):
                    detected = pd.DataFrame({
                        ...
                        "Detected Type": [
                            str(source_df[c].dtype)
                            for c in source_df.columns
                        ],
                        "Non-Null Values": [
                            int(source_df[c].notna().sum())
                            for c in source_df.columns
                        ],
                        "Null Values": [
                            int(source_df[c].isna().sum())
                            for c in source_df.columns
                        ],
                    })
        ...
        # COLUMN MAPPING
        # ============================================================
        elif step == "🔀 Column Mapping":
            section("🔀", "Column Mapping")

            source_df = st.session_state.get("onboarding_source_df")

            if source_df is None:
                st.info("Load source data before configuring column mappings.")

            else:
                mapping = (
                    st.session_state.get("onboarding_column_mapping")
                    or ob_suggest_column_mapping(source_df)
                )

                target_options = ["DO_NOT_IMPORT"] + list(_OB_TARGET_FIELDS.keys())
                updated_mapping = {}

                st.caption(
                    "Automatic mappings are suggestions. Confirm every mapping "
                    "before staging."
                )

                for index, source_column in enumerate(source_df.columns):
                    sample_values = (
                    ...
                    existing_target = mapping.get(
                        source_column,
                        "DO_NOT_IMPORT",
                    )

                    if existing_target not in target_options:
                        existing_target = "DO_NOT_IMPORT"

                    mc1, mc2, mc3 = st.columns([1.2, 1, 1.6])
                    ...
                    with mc2:
                        selected_target = st.selectbox(
                            "Target Field",
                            target_options,
                            index=target_options.index(existing_target),
                            key=f"ob_map_target_{index}",
                        )

                    with mc3:
                        st.text_input(
                            "Sample Values",
                            " | ".join(sample_values) or "(no values)",
                            disabled=True,
                            key=f"ob_map_sample_{index}",
                        )

                    updated_mapping[source_column] = selected_target

                st.session_state["onboarding_column_mapping"] = updated_mapping

                mapped_targets = {
                    x for x in updated_mapping.values()
                    if x != "DO_NOT_IMPORT"
                }

                missing_required = [
                    target
                    for target, definition in _OB_TARGET_FIELDS.items()
                    if definition["required"] and target not in mapped_targets
                ]

                if missing_required:
                    st.error(
                        "Required target fields are not mapped: "
                        + ", ".join(missing_required)
                    )

                if st.button(
                    "Confirm Column Mapping",
                    key="ob_confirm_mapping",
                    use_container_width=True,
                    disabled=bool(missing_required),
                ):
                    ok, err = ob_save_mapping(
                        st.session_state["onboarding_batch_id"],
                    ...
                    if ok:
                        st.session_state["onboarding_mapping_confirmed"] = True
                        st.success("Column mapping confirmed and saved.")
                    else:
                        st.error(f"Mapping could not be saved: {err}")

                # ============================================================
        # DETERMINISTIC DATA VALIDATION
        # ============================================================
        elif step == "🛡️ Data Validation":
            section("🛡️", "Data Validation Engine")

            source_df = st.session_state.get("onboarding_source_df")
            mapping = st.session_state.get("onboarding_column_mapping") or {}

            if source_df is None:
                st.info("Load source data first.")

            elif not st.session_state.get("onboarding_mapping_confirmed"):
                st.warning("Confirm Column Mapping before validation.")

            else:
                if st.button(
                    "Run Deterministic Validation",
                    key="ob_run_validation",
                    use_container_width=True,
                ):
                    findings, normalized_df, summary = ob_validate_dataframe(
                        source_df,
                        mapping,
                    )

                    st.session_state["onboarding_validation_results"] = findings
                    st.session_state["onboarding_validation_summary"] = summary
                    st.session_state["_ob_normalized_df"] = normalized_df

                    ok, err = ob_persist_validation(
                        st.session_state["onboarding_batch_id"],
                        st.session_state["workspace_id"],
                        findings,
                        summary,
                    ...
                    if not ok:
                        st.error(
                            f"Validation ran but results could not be persisted: {err}"
                        )

                summary = st.session_state.get(
                    "onboarding_validation_summary"
                )
                ...
                if summary:
                    ...
                    v1.metric("Total Rows", summary["total_rows"])
                    v2.metric("Valid Rows", summary["valid_rows"])
                    v3.metric("Invalid Rows", summary["invalid_rows"])
                    v4.metric("Warning Rows", summary["warning_rows"])
                    v5.metric("Validation Score", f"{summary['score']:.1f}%")

                    if summary["status"] == "VALID":
                        st.success("✅ VALID")

                    elif summary["status"] == "VALID_WITH_WARNINGS":
                        st.warning("⚠️ VALID WITH WARNINGS")

                    else:
                        st.error("❌ INVALID — promotion is blocked")

                    if findings:
                        finding_df = pd.DataFrame(findings).rename(
                            columns={
                                "row_number": "Row",
                                "column_name": "Column",
                                "rule_code": "Rule",
                        ...
                        st.success("No deterministic validation findings.")

                # ============================================================
        # AI DATA QUALITY REVIEW
        # ============================================================
        elif step == "🧠 AI Data Quality":
            section("🧠", "AI Data Quality Review")

            normalized_df = st.session_state.get("_ob_normalized_df")
            summary = st.session_state.get("onboarding_validation_summary")
            findings = st.session_state.get(
                "onboarding_validation_results"
            ) or []

            if normalized_df is None or summary is None:
                st.info("Run deterministic validation first.")

            else:
                if st.button(
                    "Run AI Data Quality Review",
                    key="ob_run_ai_quality",
                    use_container_width=True,
                ):
                    payload = ob_ai_data_quality_review(
                        st.session_state["onboarding_batch_id"],
                        normalized_df,
                        findings,
                        summary,
                ...
                if ai_payload:
                    render_ai_score_card(
                        ai_payload,
                        title_prefix="Data Quality Readiness",
                    )
                else:
                    st.caption("AI review has not been run.")

                st.info(
                    "AI data-quality analysis is advisory only. "
                    "Deterministic validation controls production eligibility."
                )
        ...
        # SNOWFLAKE STAGING
        # ============================================================
        elif step == "❄️ Snowflake Staging":
            section("❄️", "Snowflake Staging")

            summary = st.session_state.get("onboarding_validation_summary")
            normalized_df = st.session_state.get("_ob_normalized_df")

            if not st.session_state.get("onboarding_mapping_confirmed"):
                st.warning("Column Mapping must be confirmed.")

            elif summary is None:
                st.warning("Run deterministic validation first.")

            elif summary["status"] == "INVALID":
                st.error(
                    "Staging is blocked because deterministic validation "
                    "contains errors."
                )

            elif normalized_df is None:
                st.error("Normalized source data is unavailable.")

            else:
                st.success(
                    "Validation gate passed. Data is eligible for staging."
                )

                if st.button(
                    "Write Valid Rows to Snowflake Staging",
                    key="ob_write_staging",
                    use_container_width=True,
                ):
                    source_system = (
                        "EXTERNAL_COST_MODEL"
                        if _mode == "INTEGRATED"
                        else st.session_state.get("onboarding_source_name")
                    )

                    ok, message = ob_stage_normalized_data(
                        st.session_state["onboarding_batch_id"],
                        st.session_state["workspace_id"],
                        normalized_df,
                        _mode,
                    ...
                    if ok:
                        st.session_state[
                            "onboarding_staging_complete"
                        ] = True
                        st.success(message)
                    else:
                        st.error(message)

                batch_id = st.session_state.get("onboarding_batch_id")

                if batch_id:
                    staging_preview = _wf_df(f"""
                        SELECT *
                        FROM {_OB_STG}.STG_PRICING_INPUTS
                        WHERE BATCH_ID = '{_sql_escape(batch_id)}'
        ...
        # PRODUCTION PROMOTION
        # ============================================================
        elif step == "🚀 Production Promotion":
            section("🚀", "Production Promotion")
            ...
            prerequisites = {
                "Workspace initialized": bool(workspace_id),
                "Batch created": bool(batch_id),
                "Column mapping confirmed": bool(
                    st.session_state.get("onboarding_mapping_confirmed")
                ),
                "Deterministic validation passed": bool(
                    summary
                ...
                "No blocking validation errors": bool(
                    summary and summary["invalid_rows"] == 0
                ),
                "Snowflake staging completed": bool(
            ...
            gate_df = pd.DataFrame([
                {
                    "Promotion Requirement": name,
                    "Status": "✅ Ready" if passed else "❌ Missing",
                }
                for name, passed in prerequisites.items()
            ])
            ...
            promotion_ready = all(prerequisites.values())

            if not promotion_ready:
                st.warning(
                    "Production promotion is blocked until all required "
                    "conditions are satisfied."
                )

            if st.button(
                "Promote Staged Data to Production",
                key="ob_promote_production",
                use_container_width=True,
                disabled=not promotion_ready,
            ):
                ok, message = ob_promote_batch(
                    batch_id,
                    workspace_id,
                    _mode,
                    _ob_user,
            ...
            if batch_id:
                promotion_history = _wf_df(f"""
                    SELECT
                        ...
                        COMPLETED_AT,
                        ERROR_MESSAGE
                    FROM {_OB}.DATA_PROMOTION_LOG
                    WHERE BATCH_ID = '{_sql_escape(batch_id)}'
                    ORDER BY STARTED_AT DESC
                """)

                if not promotion_history.empty:
                    st.dataframe(
                        promotion_history,
                        use_container_width=True,
                        hide_index=True,
        ...
        # ---- 7. WORKSPACE SUMMARY ----
        elif step == "✅ Workspace Summary":
            section("✅", "Customer Workspace Summary")
            product = st.session_state.get(
                "onboarding_selected_product"
            )
            ...
            readiness_checks = {
                "Customer information": bool(info.get("customer_name")),
                "Workspace initialized": bool(
                    st.session_state.get("workspace_id")
                ),
                "Snowflake connected": bool(
                    st.session_state.get("_ob_sf_ok")
                ),
                "Data source loaded": bool(
                    st.session_state.get("onboarding_batch_id")
                ),
                "Column mapping confirmed": bool(
                    st.session_state.get(
                        "onboarding_mapping_confirmed"
                    )
                ),
                "Deterministic validation passed": bool(
                    summary
                ...
                "Production promotion completed": bool(
                    st.session_state.get(
                        "onboarding_promotion_complete"
                    )
                ),
            ...
            if _mode == "INTEGRATED":
                readiness_checks["Cost Model integration"] = bool(
                    st.session_state.get("integration_ready")
                )

            readiness = round(
                sum(readiness_checks.values())
                / len(readiness_checks)
                * 100,
                0,
            )

            ready_for_pricing = all(readiness_checks.values())
            ...
            ws2.metric(
                "Workspace",
                st.session_state.get("workspace_id") or "—",
            )
            ws3.metric("Mode", _mode)
            ...
            ws5.metric(
                "Validation",
                summary["status"] if summary else "Not Run",
            )
            ws6.metric(
                "Production",
                (
            ...
            st.markdown(
                health_bar_html(
                    int(readiness),
                    ready_color,
                    f"Readiness {int(readiness)}% · {ready_label}",
                ),
                unsafe_allow_html=True,
            )
            ...
            if st.button(
                ...
                key="ob_proceed_pricing_v2",
                use_container_width=True,
                disabled=not ready_for_pricing,
            ):
                promoted = _wf_df(f"""
                    SELECT *
                ...
                if promoted.empty:
                    st.error(
                        "The promoted production input could not be loaded."
                    )

                else:
                    selected = promoted.iloc[0].to_dict()

                    # Persist onboarding completion and governed context.
                    st.session_state["onboarding_completed"] = True

                    st.session_state[
                        "onboarding_selected_customer"
                    ] = selected.get("CUSTOMER_ID")
                    ...
                    st.session_state[
                        "standalone_pricing_inputs"
                    ] = {
                        "product_cost": selected.get("PRODUCT_COST"),
                        "selling_price": selected.get("SELLING_PRICE"),
                        "target_margin": selected.get("TARGET_MARGIN"),
                        "discount": selected.get("DISCOUNT_PERCENT"),
                        "freight": selected.get("FREIGHT"),
                        "packaging": selected.get("PACKAGING"),
                        "taxes": selected.get("TAX_PERCENT"),
                        "other_charges": selected.get("OTHER_CHARGES"),
                        "currency": selected.get("CURRENCY"),
                        "customer_segment": selected.get(
                            "CUSTOMER_SEGMENT"
                        ),
                        "sales_channel": selected.get(
                            "SALES_CHANNEL"
                        ),
                    ...
                    ob_log(
                        ...
                        (
                            f"Batch="
                            f"{st.session_state['onboarding_batch_id']}; "
                    ...
                    # Do not modify main_nav_tab directly here because its
                    # radio widget has already been instantiated in this run.
                    # Queue the navigation for the beginning of the next run.
                    st.session_state[
                        "pending_main_nav_tab"
                    ] = "💰 Price Engine"
...
if _active_tab == "💰 Price Engine":
    section("🔍", "Price Optimization Engine")
        # ── Onboarding hand-off (non-invasive): seed existing widget defaults ──
    _ob_prod = st.session_state.get("onboarding_selected_product")
    _ob_cust = st.session_state.get("onboarding_selected_customer")

    if st.session_state.get("onboarding_completed"):
        if _ob_prod and "pe_sku" not in st.session_state:
            st.session_state["pe_sku"] = _ob_prod

        if _ob_prod and "cp_sku" not in st.session_state:
            st.session_state["cp_sku"] = _ob_prod

        if _ob_cust and "cp_cust" not in st.session_state:
            st.session_state["cp_cust"] = _ob_cust

        _mp = st.session_state.get("standalone_pricing_inputs")

        st.success(
            f"Onboarded workspace **{st.session_state.get('workspace_id','')}** "
            f"loaded — product {_ob_prod or '—'}"
            + (f", customer {_ob_cust}" if _ob_cust else "")
            + "."
        )

        if _mp:
            with st.expander(
                "✍️ Standalone manual pricing inputs (from Onboarding)"
            ):
                st.json(_mp)
    ...
    sku_options = (
        sku_df["SKU"].dropna().astype(str).unique().tolist()
        if not sku_df.empty
        else []
    )

    # Apply promoted onboarding SKU only if it exists in the existing
    # Price Engine product list.
    _pending_pe_sku = st.session_state.pop(
        "_pending_pe_sku",
        None,
    )

    if _pending_pe_sku:
        if _pending_pe_sku in sku_options:
            st.session_state["pe_sku"] = _pending_pe_sku
        else:
            st.warning(
                f"The onboarded SKU '{_pending_pe_sku}' was promoted successfully "
                "but does not exist in CALCULATED_STANDARD_COSTS. "
                "It cannot be selected in the current Price Engine."
            )

    # Protect the selectbox if a previous state value is no longer valid.
    if (
        st.session_state.get("pe_sku") is not None
        and st.session_state.get("pe_sku") not in sku_options
    ):
        st.session_state.pop("pe_sku", None)

    col1, col2 = st.columns([1.2, 1])

    with col1:
        if sku_options:
            selected_sku_pe = st.selectbox(
                "Select Product SKU",
                sku_options,
                key="pe_sku",
            )
        else:
            selected_sku_pe = None
            st.info("No products are available in CALCULATED_STANDARD_COSTS.")
    ...
    try:
        ...
        _pending_cp_sku = st.session_state.pop(
            "_pending_cp_sku",
            None,
        )

        if _pending_cp_sku:
            if _pending_cp_sku in sku_options:
                st.session_state["cp_sku"] = _pending_cp_sku
            else:
                st.warning(
                    f"The onboarded SKU '{_pending_cp_sku}' is not available "
                    "in the Customer Price Lookup product list."
                )

        _pending_cp_cust = st.session_state.pop(
            "_pending_cp_cust",
            None,
        )

        if _pending_cp_cust:
            if _pending_cp_cust in customer_options:
                st.session_state["cp_cust"] = _pending_cp_cust
            else:
                st.warning(
                    f"The onboarded customer '{_pending_cp_cust}' does not yet "
                    "exist in CUSTOMER_AGREEMENTS. The onboarding record remains "
                    "available, but Customer Price Lookup cannot preselect it."
                )

        if (
            st.session_state.get("cp_sku") is not None
            and st.session_state.get("cp_sku") not in sku_options
        ):
            st.session_state.pop("cp_sku", None)

        if (
            st.session_state.get("cp_cust") is not None
            and st.session_state.get("cp_cust") not in customer_options
        ):
            st.session_state.pop("cp_cust", None)

        with col_a:
            if sku_options:
                selected_sku_cp = st.selectbox(
                    "Product SKU",
                    sku_options,
                    key="cp_sku",
                )
            else:
                selected_sku_cp = None
        ...
        with col_b:
            if customer_options:
                ...
                selected_cust = None
                st.info("No customers are available in CUSTOMER_AGREEMENTS.")
